...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const updatePointerTarget = () => {
      ...
      const inside =
        pointerClientX >= bounds.left && pointerClientX <= bounds.right && pointerClientY >= bounds.top && pointerClientY <= bounds.bottom;
      if (!inside) {
        targetPresence = 0;
        requestRender();
        return;
    ...
    function render(now: number) {
      ...
      const pointerSettling =
        Math.abs(targetX - mouseX) > 0.001 || Math.abs(targetY - mouseY) > 0.001 || Math.abs(targetPresence - cursorPresence) > 0.001;
      if (pointerSettling) requestRender();
      else lastNow = null;
      requestRender();
    }
    requestRender();
