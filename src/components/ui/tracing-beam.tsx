...
export const TracingBeam = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  ...
  return (
    <motion.div ref={ref} className={cn("relative mx-auto h-full w-full", className)}>
      <div className="absolute -left-4 top-3 md:-left-14">
        <motion.div
          transition={{ duration: 0.2, delay: 0.5 }}
          animate={{ boxShadow: scrollYProgress.get() > 0 ? "none" : "rgba(0, 0, 0, 0.24) 0px 3px 8px" }}
          className="ml-[27px] flex h-4 w-4 items-center justify-center rounded-full border border-line shadow-sm"
        >
          <motion.div
            transition={{ duration: 0.2, delay: 0.5 }}
            animate={{
              backgroundColor: scrollYProgress.get() > 0 ? "#0C0D11" : "#CCEC7B",
              borderColor: scrollYProgress.get() > 0 ? "#24272f" : "#CCEC7B",
            }}
        ...
        <svg viewBox={`0 0 20 ${svgHeight}`} width="20" height={svgHeight} className="ml-4 block" aria-hidden="true">
          ...
          />
          <defs>
            <motion.linearGradient id="tracing-gradient" gradientUnits="userSpaceOnUse" x1="0" x2="0" y1={y1} y2={y2}>
              <stop stopColor="#CCEC7B" stopOpacity="0" />
              <stop stopColor="#CCEC7B" />
              <stop offset="0.5" stopColor="#E8A659" />
              <stop offset="1" stopColor="#E8A659" stopOpacity="0" />
