...
const LINE_ACTIVE = { r: 204, g: 236, b: 123, a: 0.85 };
const NODE_ACTIVE = { r: 204, g: 236, b: 123, a: 1 };
const GLOW = "204,236,123";
const RIPPLE_COLOR = "232,166,89";

function lerpN(a: number, b: number, t: number) {
  return a + (b - a) * t;
}

function lerpColor(base: typeof LINE_BASE, active: typeof LINE_BASE, t: number): string {
...
export function KineticGrid({ className }: { className?: string }) {
    ...
    (now: number) => {
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          ctx.fillStyle = lerpColor({ r: 255, g: 255, b: 255, a: 0.16 }, NODE_ACTIVE, t);
          ctx.fill();
        }
      }
      ...
      for (const r of ripples) {
        ...
        ctx.strokeStyle = `rgba(${RIPPLE_COLOR},${(r.opacity * 0.3).toFixed(3)})`;
        ctx.lineWidth = 1.4;
        ctx.stroke();
      }
    },
  ...
  useEffect(() => {
    ...
    const onClick = (e: MouseEvent) => {
      const rect = wrapper.getBoundingClientRect();
      ripplesRef.current.push({ x: e.clientX - rect.left, y: e.clientY - rect.top, radius: 0, opacity: 1, born: performance.now() });
    };
  ...
  return (
    <div ref={wrapperRef} className={cn("pointer-events-auto absolute inset-0 overflow-hidden", className)}>
